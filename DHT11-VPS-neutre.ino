...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  WiFiMulti.addAP("Sgs7ed", "dta123456");     // Connexion au réseau wifi
...
void loop() {
  ...
  if (isnan(h) || isnan(t)) {         // test si des valeurs ont été récupérées
    ...
    if((WiFiMulti.run() == WL_CONNECTED)) {   // Si le wifi est connecté
          ...
          http.begin("http://51.75.126.38/DHT11/store_temp.php");   // connexion au serveur
          
          http.addHeader("Content-Type", "http://51.75.126.38/DHT11/data.json");    // envoi d'un entête pour préciser notre format de données : json
